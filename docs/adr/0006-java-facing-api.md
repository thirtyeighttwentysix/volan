...
- Two builder styles must be generated and kept in sync: Kotlin trailing-lambda receivers and Java
  `Consumer` configuration callbacks (`Function` for transactions that return a value). Both are
  generated from the same IR, so they cannot drift silently.
- The ABI dump becomes doubly useful: it is both the compatibility record and the input to the
  Java-purity check.

## M7 implementation

Java callback entry points are derived from the KotlinPoet function model, with `@JvmName` keeping
natural names in Java and `@JvmSynthetic` hiding receiver-lambda methods from Java overload resolution.
This preserves existing Kotlin call sites. Java sees no receiver functions or `Unit` selection
getters; selecting a field is a void method such as `select.email()`.

Runtime ABI dumps and generated bytecode are both checked. Synthetic compiler members and the
compiler-emitted enum `getEntries()` accessor are excluded; standard Java `values()` remains available.
The nullability contract covers the generated client and its core/runtime/dialect dependencies.
Kotlin schema/compiler tooling is a separate build-time API.

Async methods dispatch blocking JDBC work through a configurable, application-owned executor.
Transactions run the whole synchronous callback on one worker thread. Async dispatch from an active
transaction is rejected through the returned future. See [Java API](../java-api.md) for usage,
cancellation boundaries and the compiler option required to emit JSpecify type annotations.
